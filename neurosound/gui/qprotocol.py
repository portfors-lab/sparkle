...
from neurosound.gui.stim.factory import StimFactory
...
from neurosound.gui.qconstants import CursorRole
from neurosound.resources import cursors

class QProtocolTabelModel(QtCore.QAbstractTableModel):
    """Qt wrapper for :class:`ProcotolModel<neurosound.run.protocol_model.ProtocolTabelModel>`,
    ...
    def removeTest(self, position):
        """See :meth:`ProcotolModel<neurosound.run.protocol_model.ProtocolTabelModel.remove>`"""
    ...
    def insertTest(self, stim, position):
        """See :meth:`ProcotolModel<neurosound.run.protocol_model.ProtocolTabelModel.insert>`"""
        if position == -1:
    ...
    def clearTests(self):
        """See :meth:`ProcotolModel<neurosound.run.protocol_model.ProtocolTabelModel.clear>`"""
        self.beginRemoveRows(QtCore.QModelIndex(), 0, self.rowCount()-1)
        self._testmodel.clear()
        self.endRemoveRows()

    def stimulusList(self):
        """See :meth:`ProcotolModel<neurosound.run.protocol_model.ProtocolTabelModel.allTests>`"""
        return self._testmodel.allTests()

    def verify(self, windowSize=None):
        """See :meth:`ProcotolModel<neurosound.run.protocol_model.ProtocolTabelModel.verify>`"""
...
class ProtocolView(AbstractDragView, QtGui.QTableView):
    ...
    def dropped(self, item, event):
        ...
        Re-implemented from :meth:`AbstractDragView<neurosound.gui.abstract_drag_view.AbstractDragView.dropped>`
        """
    ...
    def grabImage(self, index):
        ...
        Re-implemented from :meth:`AbstractDragView<neurosound.gui.abstract_drag_view.AbstractDragView.grabImage>`
        """
    ...
    def cursor(self, pos):
        ...
        Re-implemented from :meth:`AbstractDragView<neurosound.gui.abstract_drag_view.AbstractDragView.cursor>`
        """
    ...
    def indexXY(self, index):
        ...
        Re-implemented from :meth:`AbstractDragView<neurosound.gui.abstract_drag_view.AbstractDragView.indexXY>`
        """
