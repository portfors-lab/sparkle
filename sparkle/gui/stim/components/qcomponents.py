...
from sparkle.gui.stim.components import square_parameters, vocal_parameters, sam_parameters
...
class QPureTone(QStimulusComponent):
    def paint(self, painter, rect, palette):
        ...
        painter.drawText(rect.x() + 5, rect.y() + 12, rect.width() - 5, rect.height() - 12, QtCore.Qt.AlignLeft,
                         "Pure Tone")
...
class QVocalization(QStimulusComponent):
    ...
    def paint(self, painter, rect, palette):
        if self.file() is not None:
            ...
            painter.drawLine(smallrect.x() + x, smallrect.y() + x, smallrect.x() + smallrect.width() - x,
                             smallrect.y() + smallrect.height() - x)
...
class QSAM(QStimulusComponent):
    def showEditor(self):
        editor = sam_parameters.SAMParametersWidget(self)
        return editor
